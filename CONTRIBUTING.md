# Contributing to MongrelDB Java

Thanks for taking the time to help the MongrelDB Java client. This document
...
The MongrelDB Java client uses a standard **fork → branch → pull request**
workflow on GitHub.

1. **Fork** [`visorcraft/MongrelDB-Java`](https://github.com/visorcraft/MongrelDB-Java)
   to your GitHub account.
2. **Clone** your fork and add the upstream remote:

   ```sh
   git clone git@github.com:<you>/MongrelDB-Java.git
   cd MongrelDB-Java
   git remote add upstream https://github.com/visorcraft/MongrelDB-Java.git
   ```
...
5. **Open a pull request** against `master` on `visorcraft/MongrelDB-Java`.
...
```sh
MONGRELDB_URL=http://127.0.0.1:8453 mvn -B -ntp test
```
...
- Behavior changes ship with tests. New client behavior: a unit test
  alongside the code in `src/test/`. Query wire-format changes: cover the
  exact outgoing JSON keys. Daemon-dependent coverage: a live test that
...
- **Version.** Java 11 or newer. Don't drop the minimum casually.
- **Dependencies.** No external runtime dependencies — only the Java
  standard library (`java.net.http.HttpClient`,
  `java.util.concurrent`, `com.fasterxml.jackson` is not used). New
...
- **Errors.** Throw a typed exception hierarchy (`MongrelDBException` base,
  ...
  not generic `RuntimeException`s.
- **Naming.** Idiomatic Java: `PascalCase` types, `camelCase` methods and
  fields.
- **Async.** Blocking methods have `Async` variants returning
  `CompletableFuture`.
...
- The MongrelDB Java client version (from `pom.xml` / Maven Central).
...
The MongrelDB Java client is dual-licensed under MIT OR Apache-2.0. By
